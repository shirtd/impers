...
if __name__ == '__main__':
    ...
    CACHE = True
    FCACHE = 'cache'
    FIGDIR_NAME = 'two-circle'
    # LABEL = 'voronoi' # 'delaunay'
    LABEL = sys.argv[1] if len(sys.argv) > 1 else 'delaunay'
    ...
    GRID_RES = 1024
    N_P = int(sys.argv[2]) if len(sys.argv) > 2 else 1000 # 2000
    N_Q = int(sys.argv[3]) if len(sys.argv) > 3 else 500 # 1000
    print(sys.argv, LABEL, N_P, N_Q)
    FIGDIR = '%s-p%dq%d' % (FIGDIR_NAME, N_P, N_Q)

    CIRCLE_N = 512
    CIRCLE_R = [0.2, 0.9]
    CIRCLE_C = [[0,0], [0,0]]

    BREAKS = np.linspace(0, 0.5, 11)[1:]
    ...
    for i, alpha in enumerate(BREAKS):
        ...
        plt.pause(0.1)
        if SAVE:
            if not os.path.exists(os.path.join(FIGDIR, LABEL)):
                os.mkdir(os.path.join(FIGDIR, LABEL))

            fall = os.path.join(FIGDIR, LABEL, 'all_%s%da%de-2.png' % (LABEL, i, int(alpha*100)))
            print('saving %s' % fall)
            plt.savefig(fall, dpi=300)
            for x,l in zip(ax, ('max', 'min', 'image')):
                if not os.path.exists(os.path.join(FIGDIR, LABEL, l)):
                    os.mkdir(os.path.join(FIGDIR, LABEL, l))
                f0name = os.path.join(FIGDIR, LABEL, l, '%s-delta_%s%da%de-2.png' % (l, LABEL, i, int(alpha*100)))
                print('saving %s' % f0name)
                plt.savefig(f0name, bbox_inches=x[0].get_window_extent().transformed(fig.dpi_scale_trans.inverted()), dpi=300)
                f1name = os.path.join(FIGDIR, LABEL, l, '%s_ext_%da%de-2.png' % (l, i, int(alpha*100)))
                plt.savefig(f1name, bbox_inches=x[1].get_window_extent().transformed(fig.dpi_scale_trans.inverted()), dpi=300)
                print('saving %s' % f1name)
                f2name = os.path.join(FIGDIR, LABEL, l, '%s+delta_%s%da%de-2.png' % (l, LABEL, i, int(alpha*100)))
                print('saving %s' % f2name)
                plt.savefig(f2name, bbox_inches=x[2].get_window_extent().transformed(fig.dpi_scale_trans.inverted()), dpi=300)
        else:
